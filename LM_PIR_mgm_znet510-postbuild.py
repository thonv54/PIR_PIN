...
def S37():
    ...
    if ("iar" == "iar" or "iar" == ""):
        ...
        GCC_TOOLCHAIN_PATH = "file:/C:/Program%20Files%20(x86)/IAR%20Systems/Embedded%20Workbench%207.5/".replace(
            "file:", "").replace("%20", " ")
    ...
    os.system(MOTOROLLA_32_BIT_COMMAND)
    outputFile.write(os.popen(MOTOROLLA_32_BIT_COMMAND).read())
...
def EBL():
    ...
    print " "
    EBL_COMMAND = START_FLAGS + r"C:\SiliconLabs\SimplicityStudio\v4\developer\adapter_packs\commander\commander.exe".replace(
        " ", "\ ") + " ebl create " + EBL_FILE + " --app " + S37_BINARY + " --device EFR32MG1P132F256GM48 " + CONVERT_FLAGS
    os.system(EBL_COMMAND)
    outputFile.write(os.popen(EBL_COMMAND).read())
...
def GBL():
    ...
    GBL_COMMAND = START_FLAGS + r"C:\SiliconLabs\SimplicityStudio\v4\developer\adapter_packs\commander\commander.exe".replace(
        " ", "\ ") + " gbl create " + GBL_FILE + " --app " + S37_BINARY + " --device EFR32MG1P132F256GM48"
    os.system(GBL_COMMAND)
    outputFile.write(os.popen(GBL_COMMAND).read())
...
def OTA():
    ...
    os.system(OTA_COMMAND)
    outputFile.write(os.popen(OTA_COMMAND).read())
